@echo off
cd /d "%~dp0"
REM Command-line alternative to the app window (double-click Spectari.exe
...
REM Run Spectari.exe --list-windows to see what's capturable.
REM Whole monitor instead: replace --window "..." with --monitor 0
REM Weak upload? Lower --bitrate to 8000.

set PORT=8093

Spectari.exe --window "Path of Exile 2" --fps 60 --bitrate 12000 --port %PORT%
pause
